...
#   - Matrix/scalar/embed LR, Muon momentum warmup, grad clipping, MLP leak, and
#     warmdown fraction now match the validated bigbag defaults where our code supports them.
#
...
MLP_MULT="${MLP_MULT:-4}" \
MLP_NEGATIVE_SLOPE="${MLP_NEGATIVE_SLOPE:-0.5}" \
...
WARMDOWN_FRAC="${WARMDOWN_FRAC:-0.72}" \
